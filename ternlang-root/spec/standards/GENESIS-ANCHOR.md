...
Ternary logic introduces a mathematically pure "Pending/Unknown" State (deliberative hold). However, if an isolated ternary system is disconnected from the global consensus network, bad actors or hardware degradation could artificially force a deliberative hold equilibrium into a catastrophic State -1 (Reject) drift.
...
3. If the VM is offline, air-gapped, or running on an unauthorized fork that cannot reach the Fly.io API, **the hardware gracefully enters a permanent `THOLD` (deliberative hold)**.
