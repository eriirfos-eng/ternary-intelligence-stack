...
\title{Ternlang: A Full-Stack Balanced Ternary Execution Architecture
       for Sparse Neural Inference and Ambiguity-Aware Agent Systems\\[4pt]
       {\large\textit{The Autonomous Substrate --- The Death of the Trit}}}

\author{Simeon~Kepp,~Zabih~Karimi,~Nikoletta~Csonka,~Lisa~Scharler,~Louis~Paul~Ehrig%
...
\thanks{Contact: s.kepp@ternlang.com}%
}
...
Albert MoE-13 v2.0.0 is the reference implementation of the ideas developed in
Sections~\ref{sec:sparse}--\ref{sec:moe}: a fully ternary Mixture-of-Experts
language model trained end-to-end without GPU hardware. It serves as an empirical
existence proof that the ternary execution substrate enables practical neural
training and inference on commodity consumer hardware.
...
\centering
\small
\caption{Albert MoE-13 v2.0.0 architecture}
...
Active experts per token & 3 (Top-3 sparse) \\
Depth & 3 layers (3L training phase) \\
Total parameters & $\approx$22M \\
Active parameters per token & \textbf{7.6M} (34.5\%) \\
Weight format & ternary $\{-1, 0, +1\}$ throughout \\
Checkpoint size (float32 on disk) & 91\,MB \\
Packed ternary deployment estimate & $\approx$6\,MB \\
\bottomrule
\end{tabular}
\end{table}

All 22 million parameters are stored as discrete values $w \in \{-1, 0, +1\}$.
Top-3 sparse routing over 12 experts means 7.6M parameters are active per
forward pass---a 65.5\% parameter reduction per inference step, realised by the
same \texttt{TSPARSE\_MATMUL} mechanism described in Section~\ref{sec:sparse}.
...
\centering
\small
\caption{Albert MoE-13 v2.0.0 --- 16-epoch training summary}
...
The all-time best of 2.1353 (Epoch 9, single batch) corresponds to an effective
perplexity of $\approx 8.5$ on that batch---achieved when a specialised expert's
routing aligned precisely with a repetitive passage from the King James Bible
corpus. This demonstrates that individual MoE experts have reached genuine
pattern mastery on specific token distributions; the trit is not approximating
a distribution, it is encoding a fact.

The loss variance pattern is characteristic of healthy MoE training: variance
expands as experts differentiate (Epochs 4--10, $\sigma \approx 0.54$) then
partially compresses as routing stabilises (Epoch 11, $\sigma = 0.45$). The floor
of achievable loss per batch has monotonically decreased across all 16 epochs.

The training hardware is intentionally unremarkable: a 2013 HP ZBook, an Intel
CPU, and no GPU. This is a design goal, not a constraint. If a 22M-parameter
ternary MoE model can be trained to expert-level pattern mastery on a decade-old
consumer CPU, the case for ternary as a democratising inference substrate is
...
We have presented Ternlang v2.0.0---\textit{The Autonomous Substrate}---a
...
\begin{enumerate}
  ...
  \item \textbf{Autonomous training}: Albert MoE-13 v2.0.0 (22M ternary
        parameters, Top-3/12 MoE, 256H) achieved an all-time best cross-entropy
        of \textbf{2.1353} (perplexity $\approx$8.5) after 16 global epochs of
        autonomous training---including one self-triggered $3L \to 5L$ depth
        expansion via the EvolutionManager---running entirely on a 2013 consumer
        CPU with no GPU. Hardware-verified AVX2 SIMD benchmarks confirm
        $1.80\times$ throughput at 50\% sparsity and $3.0\times$ over INT8
        at 90\% sparsity.
\end{enumerate}

The title of this paper carries a provocation: \textit{The Death of the Trit}.
It is not an obituary. It is a claim about what happens when ternary computation
matures: like the binary bit before it, the trit stops being a novelty and
becomes invisible infrastructure. The goal of the Ternary Intelligence Stack
