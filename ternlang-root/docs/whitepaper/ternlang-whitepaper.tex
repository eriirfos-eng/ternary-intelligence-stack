...
\centering
\small
\caption{Albert MoE-13 v3.0 --- current state (multilingual, 32k vocab, as of 2026-05-12)}
\label{tab:albert_v3}
\begin{tabular}{lr}
\toprule
Metric & Value \\
...
Surgery gate gap & \textbf{0.017 nats} from 9.8 — imminent \\
...
\end{axis}
\end{tikzpicture}
\caption{Full loss trajectory for Albert MoE-13 across v2.0.0 and v3.0. Three
distinct v3.0 phases are visible: (1) post-vocabulary-surgery plateau near $\ln(32000)$
(Ep55--82); (2) Nash equilibrium freeze at Ep83--104 where expert weights became
identical and gradient flow through expert MLPs ceased; (3) sustained $-0.001$\,nats/epoch
descent from Ep105 onward after introduction of expert seed biases. The dotted green
line marks the Net2Net surgery gate (loss~$\leq$~9.8). Best epoch-average:
10.3262 (Ep115); best single batch: 10.3023 (Ep139).}
...
visualisation of gradient flow and expert routing across all 17 layers. The view renders
the model as a directed graph: INPUT at L0, OUTPUT at L16, with per-expert connections
colour-coded by routing weight and gradient magnitude. Cross-layer connections reveal
which layers are actively reorganising (dense, bright connections) versus structurally
stable (sparse, dim). The cursor-hover tooltip reports the cosine similarity of any
expert connection in real time.
