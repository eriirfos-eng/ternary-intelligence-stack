...
Inference throughput & \textbf{11.24 tok/s} \\
Eval corpus & WikiText-2~\cite{merity2016pointer} (150\,kB, held-out) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tikzpicture}
\begin{axis}[
    width=0.92\columnwidth, height=5.8cm,
    xlabel={Global Epoch},
    ylabel={Cross-Entropy Loss},
    xlabel style={font=\footnotesize, color=terncmt},
    ylabel style={font=\footnotesize, color=terncmt},
    tick label style={font=\scriptsize, color=terncmt},
    xmin=0, xmax=68, ymin=5.5, ymax=11.4,
    grid=both, grid style={color=dkgrid, very thin},
    axis background/.style={fill=dkbg},
    axis line style={color=terncmt!50},
    tick style={color=terncmt!50},
    legend style={
        ...
        at={(0.97,0.5)}, anchor=east, row sep=1pt
    },
    clip=false,
]
...
% v3.0 plateau (multilingual, 32k vocab, from Ep55)
\addplot[color=dkcyan, thick, mark=square*, mark size=1.5pt,
    ...
    (59,10.350)(60,10.350)(61,10.354)(62,10.350)
};
\addlegendentry{v3.0 (32k vocab, 8 lang.)}

% Random baseline ln(32000) = 10.373
\addplot[color=ternlit, dashed, thick, domain=0:68, samples=2] {10.373};
\addlegendentry{$\ln(32000)\!\approx\!10.373$}

% Vocab surgery vertical marker
\draw[dashed, color=terncmt!40, thin]
    (axis cs:54, 5.5) -- (axis cs:54, 11.4);
\node[rotate=90, font=\tiny, color=terncmt!65, anchor=south]
    at (axis cs:54.4, 5.7) {vocab surgery};

% First Green expert annotation
\draw[->, color=dkgreen!70, thin, shorten >=2pt]
    (axis cs:58, 10.28) -- (axis cs:56.2, 10.355);
\node[font=\tiny, color=dkgreen!75, anchor=north]
    at (axis cs:58.5, 10.26) {first G expert (Ep56)};
\end{axis}
\end{tikzpicture}
\caption{Loss trajectories for v2.0.0 (English, 8k vocab, 16 epochs) and v3.0
(multilingual, 32k vocab, Global Epochs 55--62). The dashed line marks the
32k random baseline $H_{\text{random}}=\ln(32000)\approx 10.373$; the v3.0
plateau lies below it, confirming the model extracts multilingual structure
despite the post-surgery vocabulary transfer regime. First Green expert
(G state in TLIGHT) appears at Global Epoch~56.}
