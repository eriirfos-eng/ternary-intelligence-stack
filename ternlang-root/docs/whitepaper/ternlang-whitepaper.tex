...
expansion continued through multiple sequential surgeries from 5L to 13L---the
first documented multi-surgery trajectory in a fully ternary MoE---growing the
model to $\approx$63M parameters while preserving all prior weight structure. Third,
...
Depth & 5L & \textbf{13L} (multi-surgery) \\
Context length & 128 tokens & \textbf{256 tokens} (RoPE) \\
Total parameters & $\approx$22M & $\approx$\textbf{63M} \\
...
Metric & Value \\
\midrule
Architecture & 12L $\cdot$ 256H $\cdot$ 12E $\cdot$ Top-3 $\cdot$ 128CTX \\
Vocabulary & 32,000 tokens (ByteLevel BPE, 8 languages) \\
Corpus & 177,654,147 tokens (chaos layer: 10\%) \\
Total parameters & $\approx$58M (ternary) \\
Global epochs at v3.0 launch & $\approx$55 \\
Global epochs completed & 143+ (Modal T4 GPU) \\
Best epoch-average loss & \textbf{10.3262} (Ep115) \\
Best single-batch loss & \textbf{10.3023} (Ep139) \\
Random baseline $\ln(32000)$ & 10.373 \\
Sustained loss rate (Ep83--143) & $-0.001$ nats/epoch \\
Gradient norm unblocked (Ep111) & $7\!\times\!10^{-6} \to 7\!\times\!10^{-5}$ \\
...
Net2Net surgery gate (next milestone) & loss\_avg $\leq 9.8$ \\
\bottomrule
\end{tabular}
\end{table}
...
$\approx 0.53$\,nats improvement from the Ep143 level of 10.334) is approximately
500 epochs away. Acceleration of this trajectory---via diversity loss reactivation,
entropy-floor gate regularisation, or learning rate warm-up after surgery---is the
primary engineering target for the post-Stage-1 programme.
