...
\centering
\small
\caption{Workspace crates, test counts, and brief descriptions (v0.3.0)}
\label{tab:status}
\begin{tabular}{p{2.5cm}lp{5.5cm}}
\toprule
Crate & Tests & Description \\
\midrule
\texttt{ternlang-core}    & 43  & Lexer, parser (block comments, top-level stmt wrapping), AST, semantic checker, BET VM (51 opcodes), Tset Int coercion fix \\
...
\texttt{ternlang-codegen} & 8   & AST-to-C11 transpiler; match/struct/propagation; \texttt{Stmt::Set}, \texttt{BinOp::Div/Mod}, \texttt{Expr::FloatLiteral} coverage \\
...
\texttt{ternlang-hdl}     & 34  & Verilog-2001 codegen, BET RTL simulator (\texttt{BetRtlProcessor}), \texttt{ternlang sim --rtl} \\
...
\texttt{ternlang-mcp}     & --- & MCP server, 13 tools: \texttt{trit\_decide}, \texttt{moe\_orchestrate}, \texttt{trit\_action\_gate}, \ldots \\
...
(core: 43, codegen: 8, moe: 24, compat: 29, hdl: 34), all passing; network-bound crates
(ternlang-runtime, ternpkg, ternlang-lsp, ternlang-ml, ternlang-mcp) are validated locally.
...
binary. A companion \texttt{stdlib/} directory contains \textbf{275+ executable}
\texttt{.tern} programs spanning aerospace, medicine, distributed systems,
hardware pipelines, civic governance, finance, AI agent design, and ten Qutrit
Neural Network modules (gates, Hadamard, entanglement, Grover oracle, VQE,
QAOA)—each catalogued in \texttt{INDEX.md} with patterns, attribution, and
cross-references. The VS~Code extension (\texttt{ternlang-0.1.0}) is published
on the Open~VSX Registry under publisher \texttt{rfi-irfos}, providing syntax
highlighting, LSP diagnostics, and snippet completion to any compatible editor.
The repository is publicly available at the URL in the author affiliation.
Continuous integration via GitHub Actions automatically builds, tests, and
...
We have presented Ternlang v0.3.0, a full-stack balanced ternary execution
...
\begin{enumerate}
  ...
  \item \textbf{Deployment and tooling}: the live API at
        \texttt{https://ternlang.com/mcp} serves 13 MCP tools to any compatible
        ...
        deploys on every push; and 138 CI-verified tests across 5 fast crates
        (275+ \texttt{.tern} stdlib programs) validate the full stack.
\end{enumerate}
...
API. Near-term targets include submission to the VS~Code Marketplace and academic
