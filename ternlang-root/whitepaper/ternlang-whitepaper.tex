...
elements yields $56.2\%$ sparsity and a $2.3\times$ reduction in multiply
operations versus dense execution—without approximate arithmetic or hardware
reconfiguration.
...
\begin{itemize}
  \item \textbf{ternlang-core} (31 tests): lexer, parser, AST, semantic
        checker, BET bytecode emitter, VM
  \item \textbf{ternlang-ml} (6 tests): BitNet quantization kernels, dense
        and sparse matmul, benchmark harness
  \item \textbf{ternlang-hdl} (21 tests): Verilog-2001 codegen, FPGA
        simulation wrapper
  \item \textbf{ternlang-lsp}: LSP 3.17 language server (hover, completion,
        diagnostics)
  \item \textbf{ternlang-mcp}: MCP server with 6 tools including
        \texttt{trit\_decide}
  \item \textbf{ternlang-runtime} (2 tests): distributed TCP actor runtime
  \item \textbf{ternlang-compat} (29 tests): \texttt{.tasm} assembler,
        Owlet S-expression parser
  \item \textbf{ternpkg} (5 tests): package manager with GitHub-backed registry
\end{itemize}

The full test suite comprises \textbf{97 tests}, all passing. The repository
is publicly available at the URL in the author affiliation.

% ─────────────────────────────────────────────────────────────────────────────
\section{Conclusion and Future Work}
\label{sec:conclusion}

We have presented Ternlang, the first complete software stack for balanced
ternary computing. The central technical contribution—\texttt{TSPARSE\_MATMUL}
as a first-class ISA primitive—achieves a $2.27\times$ reduction in multiply
operations for quantized neural network weights without approximation.

Future directions include:

\begin{itemize}
  \item \textbf{TCOMPRESS/TUNPACK}: run-length compression of sparse trit
        tensors in the VM heap
  \item \textbf{FPGA synthesis}: full \texttt{bet\_processor} targeting
        Xilinx Artix-7 and Lattice ECP5
  ...
  \item \textbf{End-to-end training}: native ternlang training loop with
        BitNet-style gradient quantization
\end{itemize}

The ternary computing field has been fragmented for decades. Ternlang is
designed to be the substrate where those fragments converge.
