> **Historical document.** This report captures the system state at v1.3.5 (early 2026). albert. is now at v3.0 · ep2802 · 18L · 9.2746 epoch-ATL. Current architecture, benchmarks, and capabilities are documented in `ternlang-root/README.md` and `ternlang-root/BENCHMARKS.md`.
...
- **[VERIFIED] Full Transformer Stack:** Implemented `Embedding`, `Attention` (multi-head), `Mlp` (GELU), and residual `Block` structures.
- **[VERIFIED] Ternary-Native Forward Pass:** Integrated `TernaryLinear` layers using a 0.05 threshold and bit-ready weights.
- **[VERIFIED] STE-Driven Training:** Successfully optimized discrete ternary weights using the Straight-Through Estimator.
- **[MEASURED] Model Configuration:** `vocab_size: 8000`, `hidden_size: 64`, `num_layers: 2`, `num_heads: 4`.
...
### Copernicus-v1 Training [MEASURED]
- **Corpus:** King James Bible (1,014,621 tokens).
- **Outcome:** Demonstrated stable convergence over 20 epochs using full-vocabulary cross-entropy loss.
- **Loss Reduction:** 8.9872 (Uniform) -> 5.8799 (Convergence).
- **Artifact:** Trained full-precision weights persisted at `albert-moe-13/models/bible_ternary_v1.3.5.safetensors`.

## 4. Current Limitations
- **Model Scale (at v1.3.5):** No large-scale Transformer (e.g., Llama-3 scale) was yet implemented; the `Transformer` architecture was at 2L hidden-64. By v3.0 the system reached 18L hidden-256, multilingual 32k vocab, and continuous autonomous growth via Net2Net surgery.
- **Semantic Generation (at v1.3.5):** The system did not yet exhibit semantic language generation at this version; outputs were limited to token-repetition patterns for architectural validation. This was resolved by v3.0 (multilingual corpus, 32k BPE vocab, continuous generation).
- **Training Evidence:** **[VERIFIED]** Full-vocabulary training on the Bible corpus (1M tokens) confirmed convergence of the ternary-native Transformer architecture.
...
The immediate technical objective is the implementation of bit-packed weight loading (2 bits per weight) to enable memory-efficient deployment of the v1.3.5 Bible model, followed by the integration of the straight-through estimator (STE) into the MoE routing logic.
