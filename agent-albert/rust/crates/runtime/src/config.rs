...
pub enum McpServerConfig {
    ...
    AlbertAiProxy(McpAlbertAiProxyServerConfig),
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct McpAlbertAiProxyServerConfig {
    pub url: String,
    pub id: String,
}
...
impl ConfigLoader {
    ...
    pub fn default_for(cwd: impl Into<PathBuf>) -> Self {
        ...
        let config_home = std::env::var_os("CLAUDE_CONFIG_HOME")
            .map(PathBuf::from)
            .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".albert")))
            .unwrap_or_else(|| PathBuf::from(".albert"));
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".albert.json"),
            |parent| parent.join(".albert.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".albert.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".albert").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".albert").join("settings.local.json"),
            },
        ]
    }
...
impl McpServerConfig {
    ...
    pub fn transport(&self) -> McpTransport {
        match self {
            ...
            Self::AlbertAiProxy(_) => McpTransport::AlbertAiProxy,
        }
    }
}
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".albert.json");
...
fn parse_mcp_server_config(
    ...
    match server_type {
        ...
        "albertai-proxy" => Ok(McpServerConfig::AlbertAiProxy(
            McpAlbertAiProxyServerConfig {
                url: expect_string(object, "url", context)?.to_string(),
...
mod tests {
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".albert");
    ...
    #[test]
    fn loads_and_merges_albert_code_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".albert");
        fs::create_dir_all(cwd.join(".albert")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".albert.json"),
