...
const DEFAULT_MODEL: &str = "albert-opus-4-6";
...
fn resolve_model_alias(model: &str) -> &str {
    match model {
        "opus" => "albert-opus-4-6",
        "sonnet" => "albert-sonnet-4-6",
        "haiku" => "albert-haiku-4-5-20251213",
        _ => model,
    }
}
...
fn print_bootstrap_plan() {
    for phase in runtime::BootstrapPlan::albert_code_default().phases() {
        println!("- {phase:?}");
    }
}
...
fn default_oauth_config() -> OAuthConfig {
    OAuthConfig {
        ...
        authorize_url: String::from("https://platform.albert.com/oauth/authorize"),
        token_url: String::from("https://platform.albert.com/v1/oauth/token"),
        callback_port: None,
        manual_redirect_url: None,
        scopes: vec![
            String::from("user:profile"),
            String::from("user:inference"),
            String::from("user:sessions:albert_code"),
        ],
    }
}
...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Starting Albert OAuth login...");
    ...
    println!("Albert OAuth login complete.");
    Ok(())
}

fn run_logout() -> Result<(), Box<dyn std::error::Error>> {
    clear_oauth_credentials()?;
    println!("Albert OAuth credentials cleared.");
    Ok(())
}
...
fn wait_for_oauth_callback(
    ...
    let body = if callback.error.is_some() {
        "Albert OAuth login failed. You can close this window."
    } else {
        "Albert OAuth login succeeded. You can close this window."
    };
    let response = format!(
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".albert").join("sessions");
...
fn init_albert_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    Ok(initialize_repo(&cwd)?.render())
}

fn run_init() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", init_albert_md()?);
    Ok(())
}
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(out, "  claw --model albert-opus \"summarize this repo\"")?;
    writeln!(
        out,
...
mod tests {
    ...
    fn resolves_model_aliases_in_args() {
        ...
        assert_eq!(
            ...
            CliAction::Prompt {
                prompt: "explain this".to_string(),
                model: "albert-opus-4-6".to_string(),
                output_format: CliOutputFormat::Text,
                allowed_tools: None,
                permission_mode: PermissionMode::DangerFullAccess,
            }
    ...
    fn resolves_known_model_aliases() {
        assert_eq!(resolve_model_alias("opus"), "albert-opus-4-6");
        assert_eq!(resolve_model_alias("sonnet"), "albert-sonnet-4-6");
        assert_eq!(resolve_model_alias("haiku"), "albert-haiku-4-5-20251213");
        assert_eq!(resolve_model_alias("albert-opus"), "albert-opus");
    }
    ...
    fn model_report_uses_sectioned_layout() {
        let report = format_model_report("albert-sonnet", 12, 4);
        assert!(report.contains("Model"));
        assert!(report.contains("Current model    albert-sonnet"));
    ...
    fn model_switch_report_preserves_context_summary() {
        let report = format_model_switch_report("albert-sonnet", "albert-opus", 9);
        assert!(report.contains("Model updated"));
        assert!(report.contains("Previous         albert-sonnet"));
        assert!(report.contains("Current          albert-opus"));
    ...
    fn status_line_reports_model_and_token_totals() {
        ...
        assert!(status.contains("Model            albert-sonnet"));
 ...
 M src/main.rs",
    ...
    fn init_template_mentions_detected_rust_workspace() {
        let rendered = crate::init::render_init_albert_md(std::path::Path::new("."));
