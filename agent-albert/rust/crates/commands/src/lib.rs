...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "config",
        summary: "Inspect Albert config files or merged sections",
        argument_hint: Some("[env|hooks|model]"),
        resume_supported: true,
    },
    SlashCommandSpec {
        name: "memory",
        summary: "Inspect loaded Albert instruction memory files",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
...
mod tests {
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/model albert-opus"),
            Some(SlashCommand::Model {
                model: Some("albert-opus".to_string()),
            })
        );
        assert_eq!(
    ...
    fn ignores_unknown_or_runtime_bound_slash_commands() {
        ...
        assert!(
            handle_slash_command("/model albert", &session, CompactionConfig::default()).is_none()
        );
