...
function Download($url, $dest) {
    Write-Host "  <- $([System.IO.Path]::GetFileName($dest))"
    $wc = New-Object System.Net.WebClient
    $wc.DownloadFile($url, (Join-Path (Get-Location) $dest))
}

Write-Host "[1/5] Downloading binary (Windows x86_64)..."
Download "$Release/moe-test-windows-x86_64.exe" "$Dir\moe-test.exe"

Write-Host "[2/5] Downloading model weights (142 MB)..."
Download "$Release/bible_ternary_v2.0.0.safetensors" "$Dir\models\bible_ternary_v2.0.0.safetensors"

Write-Host "[3/5] Downloading model config..."
Download "$Release/bible_ternary_v2.0.0.config.json" "$Dir\models\bible_ternary_v2.0.0.config.json"
Download "$Release/bible_ternary_v2.0.0.meta"        "$Dir\models\bible_ternary_v2.0.0.meta"

Write-Host "[4/5] Downloading vocabulary + eval sample..."
Download "$Release/vocab.json"      "$Dir\data\vocab.json"
Download "$Release/eval_sample.txt" "$Dir\eval_sample.txt"
