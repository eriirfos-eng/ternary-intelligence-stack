# RFI-IRFOS Leadership Team

Ternlang and the Ternary Intelligence Stack (TIS) are developed by **RFI-IRFOS**, an interdisciplinary research facility based in Graz, Austria, dedicated to frontier deep technology and European technological sovereignty.

---

## The Core Team

### Simeon Kepp
**Co-Founder & Head of Research**  
*Operational Lead*

Simeon leads the architectural vision and research direction of the Ternary Intelligence Stack. His focus is on the fundamental shift from scaling-centric AI to logic-centric AI, ensuring that TIS provides a robust, causal, and transparent alternative to traditional binary models.

*   **Role:** Operational Lead, Lead Architect.
*   **Focus:** Balanced Ternary Logic, Causal Routing, Systems Architecture.
*   **LinkedIn:** [Simeon Kepp](https://www.linkedin.com/in/simeon-kepp/)
*   **Email:** s.kepp@ternlang.com

### Nikoletta Csonka
**Co-Founder & Head of International Relations**  
*Strategic Outreach*

Nikoletta manages the strategic positioning and international partnerships of RFI-IRFOS. She ensures that the Ternary Intelligence Stack aligns with European regulatory frameworks like the EU AI Act and drives the adoption of TIS across sovereign infrastructure projects.

*   **Role:** Lead Management, Strategic Outreach, Regulatory Alignment.
*   **Focus:** EU AI Act Compliance, International Relations, Strategic Partnerships.
*   **LinkedIn:** [Nikoletta Csonka](https://www.linkedin.com/in/csonikoletta/)
*   **Email:**  csonikoletta@ternlang.com
---

### Zabih Karimi
**Co-Founder & IT-Techniker**  
*Network & ML Application Specialist*

Zabih specializes in the practical implementation of the TIS stack, from networking protocols to high-performance ML applications. He oversees the technical reliability of the BET VM and the integration of MoE-13 experts into real-world sovereign environments.

*   **Role:** IT-Technician, Network Specialist, ML Application Engineer.
*   **Focus:** Networking, ML Inference, Technical Operations, BET VM Stability.
*   **LinkedIn:** [Zabih Karimi](https://www.linkedin.com/in/zabih-karimi-3292ab349/)
*   **Email:**  z.karimi@ternlang.com

---

## Our Mission

We are "Styrian Rebels" in the field of deep technology. We believe that Europe's path to technological sovereignty lies not in playing catch-up with binary hyperscalers, but in leapfrogging to the next frontier: **Post-Binary Intelligence.**

Ternlang is the first step in this journey — a full-stack ecosystem built on the honest logic of the `trit`.

**RFI-IRFOS**  
Research Focus Institute — Interdisciplinary Research Facility for Open Sciences  
Elisabethinergasse 25, 8020 Graz, Austria  
ZVR: 1015608684  
[ternlang.com](https://ternlang.com)
